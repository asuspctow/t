...
/* Minified build of https://github.com/davidjbradshaw/iframe-resizer/blob/master/src/iframeResizer.contentWindow.js
 * 
 * Three changes have been made to the original file on Github.
 * 1. A custom height calculation method is used to add 16px to the scrollHeight (i.e. the 'bodyScroll' preset). This is to prevent the 
 *    horizontal scrollbar (if it exists) from being cut off, since the scrollbar adds to the iframe height but is not factored into the 
 *    default height calculation.
 * 2. During initialisation the stopInfiniteResizingOfIframe() method is called, which sets the height of the body to be empty. 
 *   However this doesn't behave the same as a height of "auto" if the parent has styles that override it. e.g. if the height of the body 
 *   is 100% in the parent, the iframe will not resize to shrink with its content as the height will remain at 100%. Those lines
 *   have been changed to explicitly set the height to "auto".
 *      document.documentElement.style.height="auto", document.body.style.height="auto";
 * 
 * 3. The vertical scrollbar can flicker during resizing, however it should not display at all. Y overflow is set to "hidden" in the 
 *    same initialisation function as above.
 *      documentElement.style.overflowY="hidden", document.body.style.overflowY="hidden"
 */

window.iFrameResizer = {
		heightCalculationMethod: () => document.body.scrollHeight + 16
	}
	...
	! function(c) {
		...
		function ae() {
			...
			t = y.substr(M).split(":"), I = t[0], a = c !== t[1] ? Number(t[1]) : a, l = c !== t[2] ? e(t[2]) : l, E = c !== t[3] ? e(t[3]) : E, b = c !== t[4] ? Number(t[4]) : b, i = c !== t[6] ? e(t[6]) : i, u = t[7], p = c !== t[8] ? t[8] : p, r = t[9], d = t[10], R = c !== t[11] ? Number(t[11]) : R, w.enable = c !== t[12] && e(t[12]), A = c !== t[13] ? t[13] : A, P = c !== t[14] ? t[14] : P, O = c !== t[15] ? Boolean(t[15]) : O, ie("Initialising iFrame (" + window.location.href + ")"),
				...
				}(), ce("background", r), ce("padding", d), (t = document.createElement("div")).style.clear = "both", t.style.display = "block", t.style.height = "0", document.body.appendChild(t), fe(), me(), document.documentElement.style.height = "auto", document.body.style.height = "auto", document.documentElement.style.overflowY = "hidden", document.body.style.overflowY = "hidden", ie('HTML & body height set to "auto"'), ie("Enable public methods"), D.parentIFrame = {
